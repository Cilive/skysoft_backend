...
class CompanyRegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['en_name','ar_name','en_place','ar_place','en_district','ar_district',
                  'cr_no','vat_no','lan_no','email','logo','phone','password','username']
        extra_kwargs = {
...
class EmployeeRegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Employee
        # fields = ['fullname', 'username', 'email', 'phone', 'password', 'password2','id_proof','photo']
        fields = ['phone', 'username','password','email','name','iqama_no','branches']
        extra_kwargs = {
            'password':{'write_only':True}
        }
    def save(self):
        ...
        if Employee.objects.filter(phone=self.validated_data['name']).exists():
            raise serializers.ValidationError({'error':'This employee is already been used'})
